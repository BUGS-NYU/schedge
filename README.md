...
## NOTE: NYU removed the service that Schedge uses, so Schedge v1 has been deprecated, and will not have up-to-date data for semesters after Fall 2022.
The service that NYU replaced it with uses an entirely different data model, so I'll be releasing a version 2 of Schedge with breaking changes to update Schedge to the new data model.
