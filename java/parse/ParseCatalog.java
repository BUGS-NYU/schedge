    ...
    */
    ...
    private static List<Meeting> parseSectionTimesData(String times, String dates)
        ...
        for (int idx = 0; idx < timeTokens.size() / 3; idx++) {
          ...
          Duration duration = Duration.millis(endTime.getMillis() - beginDate.getMillis());

            DateTime endDate =
                    timeParser.parseDateTime(dateTokens.get(datesIdx + 1) + " 11:59PM");

          Duration activeDuration = Duration.millis(endDate.getMillis() - beginDate.getMillis());
            ...
            for (int day = 0; day < 7; day++, beginDate.plusDays(1)) {
                if (daysList.contains(DayOfWeek.of(beginDate.getDayOfWeek())))
                    meetings.add(
                            new Meeting(beginDate, duration, activeDuration.minus(Duration.standardDays(day))));
            }
        }
