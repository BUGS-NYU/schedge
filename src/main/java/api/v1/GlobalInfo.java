package api.v1;

import static utils.TryCatch.*;

import api.*;
import database.GetConnection;
import database.epochs.LatestCompleteEpoch;
import io.javalin.http.Handler;
import io.javalin.plugin.openapi.dsl.OpenApiDocumentation;
import java.util.*;
import types.*;
import utils.TryCatch;

public final class GlobalInfo extends Endpoint {
  public String getPath() { return "/info"; }

  public OpenApiDocumentation configureDocs(OpenApiDocumentation docs) {
    return docs
        .operation(openApiOperation -> {
          openApiOperation.description(
              "This endpoint provides general information on schools and subjects");
          openApiOperation.summary("Health Endpoint");
        })
        .jsonArray("200", Integer.class,
                   openApiParam -> { openApiParam.description("OK."); });
  }

  public Handler getHandler() {
    return ctx -> {
      // @TODO return information about schools, subjects, and the current term,
      // all in one.

      // https://stackoverflow.com/questions/17374743/how-can-i-get-the-memory-that-my-java-program-uses-via-javas-runtime-api

      ctx.contentType("application/json");
      ctx.json(1);
    };
  }
}
