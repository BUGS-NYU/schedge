...
/**
 * Scraping endpoint; this intentionally doesn't have API documentation, because it's a private API
 * endpoint.
 */
public final class ScrapingEndpoint {
  private static final Logger logger = LoggerFactory.getLogger("api.v1.ScrapingEndpoint");

  private static final AtomicReference<Object> MUTEX = new AtomicReference<>(null);
      ...
      var source =
          ArrayJS.run(
              () -> {
                ...
                if (src == null) src = "";
                if (src.trim().isEmpty()) src = "sis.nyu.edu";
        ...
        ws -> {
          ws.onConnect(
              ctx -> {
                ...
                if (!Arrays.equals(authHash, AUTH_HASH) || !authString.equals(AUTH_STRING)) {
                  ctx.closeSession(1000, "Failed: Unauthorized");
                  return;
                }
                ...
                CompletableFuture.runAsync(
                    () -> {
                      ...
                      try {
                        ...
                        if (closeReason.equals("Done!")) closeCode = 1000;
                      } catch (CancellationException e) { // Do nothing because job was cancelled
                        closeReason = "Job was cancelled";
                        logger.info(closeReason);
                      } catch (InterruptedException e) { // Do nothing because job was interrupted
