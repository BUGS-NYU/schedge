...
@CommandLine.Command(name = "db",  subcommands = {database.Scrape.class, database.Scrape.class,
...
public class database implements Runnable{
    ...
    public static class Scrape implements Runnable {
        ...
        public void run() {
            ...
            if(this.school == null) {
                Scrape_catalogKt.scrapeFromCatalog(term, SubjectCode.allSubjects(), batchSize).iterator().forEachRemaining(
                        course -> {
                            try {
                ...
                Scrape_catalogKt.scrapeFromCatalog(term, SubjectCode.allSubjects(school), batchSize).iterator()
                        .forEachRemaining(
                                courses -> {
                                    try {
                ...
                try {
                    InsertCourses.insertCourses(
                            term, Scrape_catalogKt.scrapeFromCatalog(term, new SubjectCode(subject, school)
                    ));
                } catch (SQLException e) {
                    e.printStackTrace();
                }
    ...
    @CommandLine.Command(
            name = "serve", sortOptions = false, headerHeading = "Usage:%n%n",
            synopsisHeading = "%n", descriptionHeading = "%nDescription:%n%n",
            parameterListHeading = "%nParameters:%n",
