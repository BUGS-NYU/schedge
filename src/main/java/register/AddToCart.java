...
public class AddToCart {
  ...
  public static void
  addToCart(User user, Term term, List<RegistrationCourse> courses) {
  ...
  public static Future<String> addToCart(User user, Term term, RegistrationCourse course,
    ...
    String optionsForm = getOptionForm(term, course, newContext);
...
//            }));
    ...
    if(sectionsForm != null) {
      ...
      Request sectionRequest =
              new RequestBuilder()
                      ...
                      .setRequestTimeout(60000)
                      .setHeader("Referer", ADD_RELATED_ROOT_URL_STRING + term.getId() + "/" + course.registrationNumber)
                      .setHeader("Host", "m.albert.nyu.edu")
                      .setHeader("Accept-Language", "en-US,en;q=0.5")
                      .setHeader("Accept-Encoding", "gzip, deflate, br")
                      .setHeader("Content-Type",
  ...
  public static Future<Void> addRelated(Term term, String form, Integer registrationNumber,
                                         Context.HttpContext context) {
     ...
     * "https://m.albert.nyu.edu/app/student/enrollmentcart/addToCart/NYUNV/UGRD/1204/7669";
     */
    System.out.println(ADD_RELATED_ROOT_URL_STRING + term.getId() + "/" + registrationNumber);
    Request sectionRequest =
            new RequestBuilder()
                    .setUri(Uri.create(ADD_RELATED_DATA_URL_STRING))
                    .setRequestTimeout(60000)
   ...
   */
  public static Future<String> addOptions(Term term, String form, Integer registrationNumber, Context.HttpContext context) {
  ...
  private static String getSectionsForm(Term term, RegistrationCourse course, Context.HttpContext context) {
    if(course.sectionsRelated.size() == 0) {return null;}
    ...
    for(int i = 0; i < course.sectionsRelated.size(); i++) {
      form += "&component" + (i + 1) + "=" + course.sectionsRelated.get(i).intValue();
    }
    return form;
  }

  //@ToDo: Change the start date based on term. Handle different start dates and different acad_career
  private static String getOptionForm(Term term, RegistrationCourse course, Context.HttpContext context) {
    if(course.units == Math.round(course.units)) {
      return String.format(
              ...
              context.csrfToken, course.waitList, course.permissionNo, Math.round(course.units) , term.getId(), course.registrationNumber
      );
    }
    return String.format(
            "CSRFToken=%s&wait_list_ok=%s&permission_nbr=%s&units=%s&" +
                    "start_date=09/02/2020&rqmnt_designtn_opt=N&instructor=" +
                    "&institution=NYUNV&acad_career=UGRD&strm=%s&class_nbr=%s&options_submit=",
            context.csrfToken, course.waitList, course.permissionNo, course.units , term.getId(), course.registrationNumber
    );
  }
