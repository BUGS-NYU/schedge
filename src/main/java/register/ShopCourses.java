...
public class ShopCourses {
  ...
  private static final String ENROLL_DATA_URL_STRING =
          "https://m.albert.nyu.edu/app/student/enrollmentcart/enroll/NYUNV/UGRD/";
  ...
  public static List<Cookie> addToCart(String username, String password,
    ...
    List<Cookie> cookies = getLoginSession(username, password, context);
  ...
  public static List<Cookie> removeFromCart(String username, String password,
    ...
    Request request2 =
            new RequestBuilder()
                    .setUri(Uri.create(SHOPPING_CART_REMOVE_DATA_URL_STRING + term.getId()))
                    .setRequestTimeout(60000)
                    .setHeader(
                            "Referer",
                            "https://m.albert.nyu.edu/app/student/enrollmentcart/cart/NYUNV/UGRD/" + term.getId())
                    .setHeader("Host", "m.albert.nyu.edu")
                    .setHeader("Accept-Language", "en-US,en;q=0.5")
                    .setHeader("Accept-Encoding", "gzip, deflate, br")
                    .setHeader("Content-Type",
  ...
  public static void enrollCourse(String username, String password,
                                  Term term, int registrationNumber,
                                  Context.HttpContext context) {
    List<Cookie> cookies = getLoginSession(username, password, context);
    String enrollForm = String.format(
            "institution=NYUNV&acad_career=UGRD&strm=%s&confirm_enrollment=N&CSRFToken=%s&enroll=&selected[]=%s",
            term.getId(), context.csrfToken, registrationNumber
    );
    System.out.println(enrollForm);
    Request request =
            new RequestBuilder()
                    .setUri(Uri.create(ENROLL_DATA_URL_STRING + term.getId()))
                    .setRequestTimeout(60000)
                    .setHeader(
                            "Referer",
                            "https://m.albert.nyu.edu/app/student/enrollmentcart/cart")
