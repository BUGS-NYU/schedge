...
public class ScheduleTest {
  ...
  public void testConflictDifferentCampuses() {
    ...
    JavalinTest.test(app, (server, client) -> {
      try (var resp = client.get(testUrl); var respBody = resp.body()) {
        ...
        Assert.assertEquals(
            Nyu.Campus.timezoneForCampus(schedule.conflictA.campus), nyc);
        Assert.assertEquals(
            Nyu.Campus.timezoneForCampus(schedule.conflictB.campus), paris);
        Assert.assertEquals(
            schedule.conflictA.beginDate.withZoneSameInstant(nyc), nyc2);
        Assert.assertEquals(
            schedule.conflictB.beginDate.withZoneSameInstant(paris), paris1);
      }
    });
  }
  ...
  public void testConflictDifferentCampusesDaylightSavings() {
    ...
    Assert.assertTrue(
        "Times as-stated should not conflict (paris1 before nyc3)",
        paris1.isBefore(nyc3));
    Assert.assertFalse(
        "Times as-stated should not conflict (paris2 before nyc3)",
        paris2.isBefore(nyc3));
    ...
    Assert.assertTrue(
        "Times after Paris DST change should conflict (paris5 before nyc8)",
        paris5.isBefore(nyc8));
    Assert.assertTrue(
        "Times after Paris DST change should conflict (nyc7 before paris6)",
        nyc7.isBefore(paris6));
    ...
    JavalinTest.test(app, (server, client) -> {
      try (var resp = client.get(testUrl); var respBody = resp.body()) {
        ...
        Assert.assertEquals(
            Nyu.Campus.timezoneForCampus(schedule.conflictB.campus), paris);
        Assert.assertEquals(
            schedule.conflictA.beginDate.withZoneSameInstant(nyc), nyc3);
        Assert.assertEquals(
            schedule.conflictB.beginDate.withZoneSameInstant(paris), paris1);
      }
    });
